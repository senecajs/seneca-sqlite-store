...
module.exports = function (options) {
  ...
  const sortquery = function (qent, q) {
    // TODO: Consider sorting by all columns that are specified
    // by the client.
    //
    // TODO: Consider eliminating duplication in the `metaquery`
    // helper.
    //
    if (q.sort$) {
      for (var sf in q.sort$) break
      var sd = q.sort$[sf] > 0 ? 'ASC' : 'DESC'
      return 'ORDER BY ' + sf + ' ' + sd
    }

    return null
  }

  const skipquery = function (qent, q) {
    // TODO: Consider eliminating duplication in the `metaquery`
    // helper.
    //
    if (typeof q.skip$ === 'number') {
      return ' OFFSET ' + q.skip$
    }

    return null
  }

  const limitquery = function (qent, q) {
    // TODO: Consider eliminating duplication in the `metaquery`
    // helper.
    //
  ...
  var deletestm = function (qent, q) {
    ...
    stm.text = (() => {
      ...
      const metastr = (() => {
        ...
        return buildMetaSql(qent, q, meta_opts)
      })()
    ...
    // TODO: This function is very similar in terms of its interface
    // and use case with the 'metaquery' function. Consider merging
    // these two functions into one.
    //
    function buildMetaSql(qent, q, opts = {}) {
      function tryFetchOverrideOrQualifier(qual) {
        const { overrides = {} } = opts
  ...
  //const init = seneca.export('entity/init') // dbg
  //var storedesc = init(seneca, options, store) // dbg
