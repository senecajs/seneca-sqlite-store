...
module.exports = function (options) {
  ...
  function savestm(ent) {
    ...
    stm.text = 'INSERT INTO ' + escape_str(table) + ' (' + escape_str(fields) +
      ') values (' + escape_str(params) + ')'
  ...
  function deletestm (qent, q) {
    ...
    stm.text = `DELETE FROM ${escape_str(table)} ${escape_str(wherestr)}`
  ...
  function loadbyid(ent_id, ent, done) {
    // TODO: The #load method is part of the public interface of the store.
    // As such, it's a bad practice to invoke a public function from a private
    // one. This will do for now, but we may want to reconsider this approach
    // in the future.
    //
