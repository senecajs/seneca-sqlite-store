...
module.exports = function (options) {
  ...
  var store = {
    ...
    save: function (args, done) {
      ...
      // TODO: Extract `is_new` into a separate function - `intern.is_new`.
      //
      var is_new = !('id' in ent)

      if (is_new && Array.isArray(args.q.upsert$)) {
        const matchable_fields = ent.fields$().concat('id')
        const upsert_on = args.q.upsert$.filter((p) => !p.includes('$'))

        if (0 < upsert_on.length) {
          // TODO: Refactor into intern.ensure_id
          //
          const new_ent_id = tryMakeIdIfRequested(args, {
            when_no_specific_id_requested: Uuid()
          })
      ...
      return buildschema(ent, function (err, schema) {
        ...
        return self.connection.run(update_query.text, update_query.values, function (err) {
          ...
          var was_updated = this.changes > 0
  ...
  var upsertstm = function (ent, new_id, upsert_on) {
    ...
    const update_kvs = (function () {
      const kvs = upsert_on.includes('id')
        ? entp_with_new_id
        : entp
      ...
      fields.forEach(function (field) {
        if (!_.isUndefined(row[field])) {
          if (schema[field] === 'timestamp') {
            ...
            var json = maybejsonfield(field, row, schema)
  ...
  var maybejsonfield = function (field, row, schema) {
    /* NOTE:
    ...
    */
    var mayTryParse = maystorejson(field, row, schema) &&
    ...
    try {
      var parsed = JSON.parse(row[field])
  ...
  var maystorejson = function (field, row, schema) {
    var coltype = schema[field]
