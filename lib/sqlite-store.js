...
module.exports = function (options) {
  ...
  var deletestm = function (qent, q) {
    ...
    stm.text = (() => {
      const sort_q = sortquery(qent, q)
      const skip_q = skipquery(qent, q)

      // TODO: More elaborate testing throughout this closure.
      //

      if (q.all$) {
        const limit_q = limitquery(qent, q)

        if (sort_q || skip_q || limit_q) {
          return `
            ...
            WHERE id IN (
              ...
              ${escapeStr(wherestr)}
              ${escapeStr(sort_q || '')}
              ${escapeStr(limit_q || '')} ${escapeStr(skip_q || '')}
            )
          `
        }
