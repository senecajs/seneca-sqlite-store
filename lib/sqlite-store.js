...
module.exports = function (options) {
  ...
  var store = {
    ...
    save: function (args, done) {
      ...
      {
        // BEGIN - working on upserts
        //

        const table = tablename(ent)
        const entp = makeentp(ent)

        console.log('entp', entp) // dbg
        ...
        */
        ...
        if (is_new && Array.isArray(query_for_save.upsert$)) {
          const upsert_on = query_for_save.upsert$
            .filter(field => field in entp)

          if (upsert_on.length > 0) {
            const cols = Object.keys(entp).map(col => escapeStr(col)).join(', ')

            const values = ent.fields$().reduce(function (h, field) {
              const placeholder = '$' + field
              h[escapeStr(placeholder)] = escapeStr(entp[field])
              return h
            }, {})
            ...
            const update_kvs = ent.fields$().map(field => {
              const placeholder = '$' + field
        ...
        const stm = {
          text:
            `insert into ${escapeStr(table)} (${cols})` +
            `values (${escapeStr(values)})` +
            `on conflict (${escapeStr(upsert_on)})` +
            `do update set ${escapeStr(params)}`,
      ...
      return buildschema(ent, function (err, schema) {
        ...
        return self.connection.run(update_query.text, update_query.values, function (err) {
          ...
          //console.log(ent) // dbg
          //console.log('save query', save_query) // dbg
          //console.log() // dbg
