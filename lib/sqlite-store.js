...
module.exports = function (options) {
  ...
  var store = {
    ...
    save: function (args, done) {
      ...
      ent.id = ent.id || ent.id$ || Uuid()


      // NOTE: We are trying to update the entity first.
      //
      const update_query = updatebyidstm(ent)

      self.connection.run(update_query.text, update_query.values, function (err) {
        if (err) {
          return done(err)
        }
        ...
        self.connection.run(save_query.text, save_query.values, function (err) {
          if (err) {
            return done(err)
          }
    ...
    native: function (args, done) {
      return done(null, self.connecation) // TODO: FIX: QUESTION: self.connection ?
    }
  }
  ...
  var selectstm = function (qent, q) {
    ...
    function selectbyids(qent, q) {
      ...
      const stm = {
        text: 'SELECT * FROM ' + escapeStr(table) + ' WHERE id IN (' + idsstm(q) + ')',
        values: q
      }
  ...
  var updatebyidstm = function (ent) {
    var stm = {}
    var table = tablename(ent)
    var fields = ent.fields$()
    var entp = makeentp(ent)
